/**
 * What the notify module offers the rest of the application.
 *
 * Both are abstract classes rather than interfaces so they double as DI
 * tokens, and both are swappable: `EmailProviderPort` is bound to the BullMQ
 * queue adapter today and could be bound to a direct provider tomorrow.
 */

export interface SendEmailParams {
  to: string;
  subject: string;
  /** Template file name under `locales/<locale>/emails/`, without extension. */
  template: string;
