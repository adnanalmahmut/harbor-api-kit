...
/**
 * Resolves role grants plus per-user overrides into one answer, memoized per
 * request and cached in Redis behind a per-user version key.
 *
 * Kept separate from `AuthorizationService` on purpose: this is read on every
 * guarded request, including inside the auth module's Better Auth hooks, while
 * `AuthorizationService` only serves the write endpoints.
 */
@Injectable()
